var fs = require('fs')
  ...
  , Ldjsonifier = require('./ldjsonifier')
...
function createReadStream(dpkgRoot, dpkg, resourceName, options){
  ...
  if(!options.foreignkeys || !fkFields.length || !options.objectMode){ //without objectMode, no support for foreignkeys
    ...
    if(options.ldjsonify){
      stream = stream.pipe(new Ldjsonifier());
    }
    return stream;
  }
...
function getResourceStream(dpkgRoot, resource, options){
  ...
  //order matters
  if('data' in resource){
    stream =  new Streamifier(resource.data, resource.format, options);
    if(options.objectMode && (resource.format === 'json' || resource.format === 'ldjson')) {
      return stream; //we return now as Streamifier could emit the (ld)json directly without need for more parsing (as done below)
    }
  ...
  //Parsing: vanilla stream -> stream in objectMode
  if(resource.format === 'csv'){
    stream = stream.pipe(binaryCSV({json:true}));

  } else if (resource.format === 'ldjson'){ //line delimited JSON
    stream = stream.pipe(split(function(row){
      if(row) {
